  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setStats([
          { label: 'Active Exams', value: data.active_exams || 0, sub: 'Today', icon: FileText, color: 'text-indigo-600', bg: 'bg-indigo-50' },
          { label: 'Total Students', value: data.total_students || 0, sub: 'Students', icon: Eye, color: 'text-indigo-600', bg: 'bg-indigo-50' },
          { label: 'Pending Reviews', value: data.pending_reviews || 0, sub: 'Urgent', icon: AlertCircle, color: 'text-red-600', bg: 'bg-red-50' },
          { label: 'Class Average', value: `${data.class_average || 0}%`, sub: '', icon: TrendingUp, color: 'text-emerald-600', bg: 'bg-emerald-50' },
        ]);
        ...
        setUpcomingExams(
          (data.upcoming_exams || []).map((ex: any) => ({
            id: ex.id,
            title: ex.title,
            time: `${new Date(ex.start_time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} - ${new Date(ex.end_time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`,
            status: new Date(ex.start_time) > new Date() ? 'Upcoming' : 'In Progress',
        ...
        setSubmissions(
          (data.recent_submissions || []).map((sub: any) => ({
            ...
            status: sub.status === 'submitted' ? 'Pending' : sub.status === 'completed' ? 'Reviewed' : sub.status,
            integrity: '—',
            date: sub.submitted_at ? new Date(sub.submitted_at).toLocaleDateString() : sub.enrolled_at ? new Date(sub.enrolled_at).toLocaleDateString() : '',
          }))
        );
      } catch (err) {
      ...
      <div className="max-w-[1600px] mx-auto pb-24 animate-slide-up">
        ...
        <div className="mb-8">
            <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-bold text-[#1E293B]">Upcoming Exams</h2>
                <button onClick={() => onNavigate('/faculty-exams')} className="text-xs font-bold text-[#4F46E5] hover:underline flex items-center gap-1">
            ...
            {upcomingExams.length > 0 ? (
                  ...
                  {upcomingExams.map((exam: any) => (
                      <div key={exam.id} className="min-w-[300px] bg-white rounded-xl border border-[#E2E8F0] p-5 shadow-sm hover:border-indigo-200 transition-colors group">
                          <div className="flex justify-between items-start mb-3">
                              ...
                              <span className="text-xs text-slate-400 font-medium">{exam.date}</span>
                          </div>
              ...
              <div className="bg-white border-2 border-dashed border-slate-200 rounded-xl p-8 text-center">
                ...
                <p className="text-sm text-slate-500">No upcoming exams. Create one to get started.</p>
              </div>
            )}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 mb-8">
            ...
            <div className="lg:col-span-2 bg-white rounded-xl border border-[#E2E8F0] p-6 shadow-[0_1px_3px_rgba(0,0,0,0.05)] flex flex-col justify-between">
                    ...
                    <div className="space-y-6">
                        <div className="flex justify-between items-center">
                            <div className="flex flex-col">
                                ...
                                <span className="text-2xl font-bold text-[#4F46E5]">{stats[3]?.value || '0%'}</span>
                            </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="bg-emerald-50 rounded-lg p-3 border border-emerald-100">
                                <span className="text-[10px] text-emerald-600 font-bold uppercase block mb-1">Total Exams</span>
                                <span className="text-lg font-bold text-emerald-800">{stats[0]?.value || 0}</span>
                            </div>
                            <div className="bg-orange-50 rounded-lg p-3 border border-orange-100">
                                <span className="text-[10px] text-orange-600 font-bold uppercase block mb-1">Students</span>
                                <span className="text-lg font-bold text-orange-800">{stats[1]?.value || 0}</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="pt-6 mt-6 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500">
                    ...
                    <button onClick={() => onNavigate('/faculty-analytics')} className="text-indigo-600 font-bold hover:underline">View Full Report</button>
                </div>
            </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-[#E2E8F0] shadow-[0_1px_3px_rgba(0,0,0,0.05)] overflow-hidden">
            ...
            {submissions.length > 0 ? (
              ...
              <div className="p-8 text-center text-slate-400 text-sm">
                No submissions yet. Students will appear here once they start taking exams.
              </div>
            )}
        </div>
...
const PerformanceLineChart = () => {
    ...
    return (
        <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-full overflow-visible">
            ...
            {[0, 1, 2, 3, 4].map(i => (
                <line key={i} x1={padding} y1={padding + (i * (height - 2*padding)) / 4} x2={width - padding} y2={padding + (i * (height - 2*padding)) / 4} stroke="#F1F5F9" strokeWidth="1" />
            ))}
            <path d={areaD} fill="url(#chartFill)" />
            <path d={pathD} fill="none" stroke="#4F46E5" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" className="drop-shadow-sm" />
            {points.map((p, i) => (
                <g key={i} className="group cursor-pointer">
                    <circle cx={p.x} cy={p.y} r="4" fill="white" stroke="#4F46E5" strokeWidth="2" className="opacity-0 group-hover:opacity-100 transition-opacity" />
                    <foreignObject x={p.x - 20} y={p.y - 35} width="40" height="30" className="opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
                        <div className="bg-slate-800 text-white text-[10px] font-bold py-1 rounded-md text-center">{p.val}%</div>
                    </foreignObject>
                </g>
            ))}
        </svg>
